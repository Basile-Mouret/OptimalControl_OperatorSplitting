For our implementation, we wanted something as fast as the original C implementation, while providing a higher level of abstraction to make development easier and faster.
This is why we chose Julia, a dynamically compiled language.
It offers a high-level syntax that makes the code easy to read, as well as a REPL (Read-Evaluate-Print Loop) that is very practical for scripting and experimenting with libraries.
Julia is also very fast; as a dynamically compiled language, it offers performance comparable to that of C and C++.
...
Still, compared to Python, Julia has a much smaller user base and is more recent.
This makes its ecosystem smaller, but for optimization and scientific work most of the necessary libraries are available.
...
We also implemented the examples from the paper in order to compare our performance with the `C` implementation.

As Julia uses a garbage collector to handle memory management, reducing allocations is crucial for performance. We preallocate the ADMM variables and workspaces as vectors, use in-place proximal operators, and rely on the cache to reuse the factorization across iterations and warm-started solves.
